...
func main() {
	...
	if len(connCfg.Host) == 0 && connCfg.Port == 0 {
		...
		_, rdsInfo, clean, err := awstest.CreateDB("db.t2.micro", 10, connCfg.User, connCfg.Password)
	...
	dbOK := false
	name := ""
	rows, _ := pool.Query("select datname from pg_database")
